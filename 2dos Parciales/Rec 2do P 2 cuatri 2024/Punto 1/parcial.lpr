...
var
   arch:text; origen,dest:ST3;
begin
     assign(arch,'Vuelos.txt'); reset(arch);
     Inicializar(mt,n);
     while not eof(arch) do
           readln(origen,dest,dest,mt[buscar(Vaero,origen),buscar(Vaero,dest)]);
end;
...
begin
     ...
     if directo <= aux then
        writeln('El precio del vuelo directo es: ',directo:8:2)
         ...
         writeln('El codigo de la escala es: ',j,' El precio final es :', aux:8:2);
end;
...
var
     ...
     Vaero,Va:TVregA;
begin
     LeerAero(Vaero,na); LeerVuelo(mt,n,Vaero);
